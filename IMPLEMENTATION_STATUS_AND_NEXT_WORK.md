...
- `lc0jax.interpretability.dynamic_prototypes` and `tools/select_dynamic_prototypes.py` select top-scoring dynamic concept pair rows plus random controls for future teachability curricula.
...
- 2026-04-27: Dynamic prototype selection tests passed; `.venv/bin/python -m pytest -q` passed with 64 tests after adding `tools/select_dynamic_prototypes.py`.
