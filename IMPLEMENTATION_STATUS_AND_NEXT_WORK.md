...
- `lc0jax.interpretability.dynamic_teachability` and `tools/export_teachability_curriculum.py` export prototype/control selections to JSONL records with target and contrast moves for downstream student-training jobs.
...
- 2026-04-27: Teachability curriculum export tests passed; `.venv/bin/python -m pytest -q` passed with 70 tests after adding `tools/export_teachability_curriculum.py`.
