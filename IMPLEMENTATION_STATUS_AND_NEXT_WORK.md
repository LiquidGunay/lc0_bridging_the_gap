...
- `lc0jax.interpretability.dynamic_evaluation` and `tools/evaluate_dynamic_concept.py` report held-out constraint and margin satisfaction for a learned dynamic direction on a held-out pair split.
...
- 2026-04-27: Held-out dynamic evaluation tests passed; `.venv/bin/python -m pytest -q` passed with 60 tests after adding `tools/evaluate_dynamic_concept.py`.
