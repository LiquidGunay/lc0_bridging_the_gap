...
- `tests/test_mcts_rollouts.py` checks PV replay, side-to-move centipawn conversion, serialization of python-chess analysis info, reconstruction of a short root move stack from history FENs, and trajectory activation records that combine pre-root history with PV continuation history without launching LC0.
- `tests/test_pair_builders.py` checks that rollout-pair JSONL plus activation shards materialize into the `differences` matrix consumed by the dynamic sparse solver while preserving root history/provenance metadata.
- `tests/test_build_mcts_pairs.py` checks that MCTS pair extraction fails fast by default on expected per-position errors, only skips them when `--skip-errors` is explicit, preserves pre-root history when given `--root-records`, and uses stable root-record-derived activation IDs that will not collide when shards are merged.
...
- `tests/test_run_dynamic_gpu_pipeline.py` checks that PGN inputs prepare root-record JSONL and route MCTS extraction through `--root-records`, while FEN-only inputs continue to use the legacy `--fens` path.
