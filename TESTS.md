...
- `tests/test_dynamic_families.py` checks dynamic concept-family clustering, per-cluster screened sparse solves, bootstrap cosine stability, and CLI artifact output.
...
- `tests/test_pair_builders.py` checks that rollout-pair JSONL plus activation shards materialize into the `differences` matrix consumed by the dynamic sparse solver while preserving root history/provenance, row-aligned teacher policy logits, and available UCI-derived row metadata.
...
- `tests/test_dynamic_teachability.py` checks JSONL curriculum export from prototype/control reports, including provenance and malformed-row failures, plus the low-rank frozen-feature teachability adapter and CLI report path.
