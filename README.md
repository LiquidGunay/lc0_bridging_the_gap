...
- Dynamic sparse concepts from precomputed rollout pairs:
  ...
  - Prefer trajectory records for LC0 112-plane inputs so PV continuations keep rolling history:
    `python tools/dump_activations.py --pb models/BT4-1024x15x32h-swa-6147500-policytune-332.pb.gz --records data/runs/<RUN_ID>/mcts_pairs/trajectory.records.jsonl --out data/runs/<RUN_ID>/activations/trajectory_flat --activation-mode flat --store-token-activations --store-policy-logits`
  - `python tools/materialize_mcts_pairs.py --pairs-jsonl data/runs/<RUN_ID>/mcts_pairs/pairs.jsonl --activations data/runs/<RUN_ID>/activations/trajectory_flat --out data/runs/<RUN_ID>/mcts_pairs/pairs.npz --mode flat --policy-logits-key policy_logits`
  ...
  - `python tools/evaluate_teachability.py --features data/runs/<RUN_ID>/mcts_pairs/pairs.train.npz --eval-features data/runs/<RUN_ID>/mcts_pairs/pairs.test.npz --curriculum data/runs/<RUN_ID>/concepts/dynamic_sparse/teachability_curriculum.jsonl --out data/runs/<RUN_ID>/concepts/dynamic_sparse/teachability_report.json --feature-key differences --logits-key policy_logits --hidden-dim 64 --steps 1000`
    This trains a lightweight frozen-feature policy adapter with KL to the stored BT4 policy logits and reports prototype-vs-random-control top-1/KL lift. Treat it as a scalable teachability baseline, not the final Schut transfer claim.
