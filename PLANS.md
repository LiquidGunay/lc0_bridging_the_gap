...
- [x] (2026-05-09 04:00Z) Integrated dynamic concept-family outputs into markdown reports: `tools/build_dynamic_concept_report.py --families` now renders aggregate family solve metrics, bootstrap stability, and per-family held-out/prototype/policy-margin artifacts discovered under `family_XXX/`. Review hardening added solver status, train margin, objective, screening, prototype max score, policy-positive columns, explicit per-family validation runbook commands, and default `families_report.json` coverage. Focused tests passed (`7 passed`), the full suite passed (`107` collected tests), and `git diff --check` passed.
- [x] (2026-05-09 04:30Z) Added a frozen-feature teachability baseline: `tools/evaluate_teachability.py` trains a low-rank JAX policy adapter from stored pair features to BT4 policy logits, compares prototype rows against random-control rows, and reports held-out top-1/KL lift. Materialized pair NPZs and train/test splits can now carry row-aligned `policy_logits` when activation dumps store policy logits. Review hardening made prototype/control training use paired seeds, marks lift unavailable unless train budgets match, reports comparable full-batch final loss, and excludes bulky logits from curriculum metadata. Focused tests passed (`29 passed`), the full suite passed (`110` collected tests), and `git diff --check` passed.
- [ ] Generate real reference manifests for the next GPU run, scale teachability evaluation, and add notebook curriculum wrappers.
- [ ] Add a weaker LC0 checkpoint or larger student network teachability run with random-prototype baselines.
